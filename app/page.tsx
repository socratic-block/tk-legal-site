export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-950 font-sans selection:bg-indigo-100 relative overflow-hidden flex flex-col">
      
      {/* Decorative Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[600px] bg-indigo-50/50 rounded-full blur-[120px] pointer-events-none" />

      {/* Navigation */}
      <nav className="relative z-10 flex justify-between items-center px-10 py-6 border-b border-slate-100">
        <div className="text-xl font-bold tracking-tighter">
          <span className="text-indigo-700">TK</span> Legal.
        </div>
        <div className="space-x-8 text-sm font-medium text-slate-700 hidden sm:flex">
          {['Services', 'About', 'Contact'].map(link => (
            <a key={link} href="#" className="relative group hover:text-indigo-800 transition-colors">
              {link}
      ...
      <main className="relative z-10 max-w-5xl mx-auto pt-24 px-6 text-center">
        <div className="inline-block mb-6 px-4 py-1.5 rounded-full border border-indigo-100 bg-indigo-50/50 text-indigo-700 text-sm font-semibold tracking-wide">
          Based in Tbilisi, Georgia 🇬🇪
        </div>
        <h1 className="text-6xl md:text-7xl font-extrabold tracking-tighter leading-tight mb-8">
          <span className="text-slate-900">Clear Legal Strategy</span> for <span className="bg-gradient-to-r from-indigo-700 to-sky-600 bg-clip-text text-transparent">Expats.</span>
        </h1>
        ...
        <button className="bg-slate-950 text-white px-10 py-4 rounded-xl font-semibold hover:bg-indigo-700 transition-all duration-300 shadow-lg shadow-indigo-100/50 flex items-center gap-2 mx-auto">
      ...
      {/* Trust Bar (New!) */}
      <div className="relative z-10 w-full border-y border-slate-100 bg-slate-50/80 mt-20">
        <div className="max-w-5xl mx-auto px-6 py-8 flex flex-wrap justify-center gap-10 md:gap-20 text-sm font-medium text-slate-500">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-24 flex-grow">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-extrabold tracking-tight text-slate-900 mb-4">How We Can Serve You</h2>
          <p className="text-slate-600 max-w-lg mx-auto">Focused expertise designed to eliminate the friction of navigating Georgian bureaucracy.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div 
              key={index} 
              className="p-10 rounded-3xl border border-slate-100 bg-white hover:bg-indigo-50/20 hover:shadow-2xl hover:shadow-indigo-100/30 hover:border-indigo-100 hover:-translate-y-2 transition-all duration-300 relative group"
            >
              <div className="text-4xl mb-6">{service.icon}</div>
              <h3 className="text-2xl font-bold mb-4 tracking-tight group-hover:text-indigo-900">{service.title}</h3>
              <p className="text-slate-700 leading-relaxed text-[17px]">
                {service.description}
              </p>
              <div className="absolute bottom-6 left-10 right-10 h-px bg-indigo-200 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
            </div>
          ))}
        </div>
      </section>

      {/* Call to Action Footer (New!) */}
      <footer className="relative z-10 bg-slate-950 text-slate-300 py-20 mt-auto">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-slate-400 mb-10 max-w-xl mx-auto">
            Book a 30-minute introductory call to discuss your specific situation, requirements, and how we can expedite your goals.
          </p>
